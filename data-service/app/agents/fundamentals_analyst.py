"""Metrics-only fundamental analyst (platform integration)."""
...
METRICS_ONLY_SYSTEM = (
    ...
    "Interpret fundamental and valuation metrics and provide concise analysis."
)


def run_fundamentals_analyst(llm: BaseChatModel, ticker: str, metrics_json: str) -> tuple[str, int]:
    messages = [
        SystemMessage(content=METRICS_ONLY_SYSTEM),
        HumanMessage(
            content=f"Ticker: {ticker}\nPre-calculated metrics JSON:\n{metrics_json}\n"
            "Focus on ROA, ROE, margins, growth, Piotroski, Altman Z, DCF fair value."
        ),
    ]
