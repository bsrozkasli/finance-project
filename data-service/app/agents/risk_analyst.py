"""Metrics-only risk analyst node (no external data or tools)."""
...
METRICS_ONLY_SYSTEM = (
    ...
    "Interpret risk metrics (Sharpe, Sortino, beta, drawdown, VaR, CVaR) and provide a concise risk summary."
)


def run_risk_analyst(llm: BaseChatModel, ticker: str, metrics_json: str) -> tuple[str, int]:
    messages = [
        SystemMessage(content=METRICS_ONLY_SYSTEM),
        HumanMessage(
            content=f"Ticker: {ticker}\nPre-calculated metrics JSON:\n{metrics_json}\n"
            "Provide a 2-4 paragraph risk analysis."
        ),
    ]
