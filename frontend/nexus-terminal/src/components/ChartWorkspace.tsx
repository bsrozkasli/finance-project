...
import { LineChart, Layout, BarChart2, TrendingUp, Settings, Maximize2, AlertCircle, RefreshCw, PenTool, Type, Trash2, MousePointer, ShieldAlert } from 'lucide-react';
...
export default function ChartWorkspace({ stocks, onSelectStock }: ChartWorkspaceProps) {
  ...
  // Replicating the "Status Code 500 Error" overlay from the Stitch mockup
  ...
  const svgDimensions = { width: 700, height: 350 };

  // Calculate coordinates for Candlesticks & lines
  const chartCoordinates = useMemo(() => {
    if (points.length === 0) return { candles: [], linePath: '', sma20Path: '', sma50Path: '' };
    ...
    const candles = points.map((p, index) => {
      const x = paddingX + (index / (points.length - 1)) * plotWidth;
      const y = svgDimensions.height - paddingY - ((p.price - minPrice) / range) * plotHeight;
      
      // Let's mock realistic open/high/low/close values centered around p.price
      const isUp = index % 2 === 0;
      const spread = p.price * 0.008;
      const open = isUp ? p.price - spread * 0.3 : p.price + spread * 0.3;
      const close = isUp ? p.price + spread * 0.4 : p.price - spread * 0.4;
      const high = Math.max(open, close) + spread * 0.5;
      const low = Math.min(open, close) - spread * 0.5;

      const yOpen = svgDimensions.height - paddingY - ((open - minPrice) / range) * plotHeight;
      const yClose = svgDimensions.height - paddingY - ((close - minPrice) / range) * plotHeight;
      const yHigh = svgDimensions.height - paddingY - ((high - minPrice) / range) * plotHeight;
      const yLow = svgDimensions.height - paddingY - ((low - minPrice) / range) * plotHeight;
    ...
    // Main line path
    const linePoints = candles.map(c => `${c.x},${c.y}`).join(' ');
    const linePath = `M ${linePoints}`;

    // Moving average helper (simple moving average over window size)
    const calculateSMA = (windowSize: number) => {
      const smaPoints = [];
      for (let i = 0; i < candles.length; i++) {
        if (i < windowSize) {
          smaPoints.push({ x: candles[i].x, y: candles[i].y });
          continue;
        }
        let sum = 0;
        for (let j = 0; j < windowSize; j++) {
          sum += candles[i - j].rawPrice;
        }
        const avg = sum / windowSize;
        const yAvg = svgDimensions.height - paddingY - ((avg - minPrice) / range) * plotHeight;
        smaPoints.push({ x: candles[i].x, y: yAvg });
      }
    ...
    return { candles, linePath, sma20Path, sma50Path, minPrice, maxPrice };
    ...
    <div className="flex-1 flex flex-col bg-bg-base overflow-hidden">
      ...
      <div className="h-12 border-b border-outline-variant bg-bg-primary flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            Tip: {chartType === 'candle' ? 'Bar (Mum)' : 'Çizgi'}
          </button>
          ...
          <button
            ...
            title="Stitch mockup hatasını simüle et"
          >
            <ShieldAlert className="w-3.5 h-3.5" />
            <span className="hidden md:inline">HATA SİMÜLATÖRÜ</span>
          </button>
        </div>
      </div>
          ...
          <div className="w-full h-full flex flex-col justify-between">
            ...
            <div className="flex items-start justify-between z-10 select-none">
              <div>
                <div className="flex items-baseline gap-2">
                  ...
                  <span className={`font-data-mono text-sm font-bold flex items-center ${activeStock?.change >= 0 ? 'text-bull-green' : 'text-bear-red'}`}>
                ...
                <div className="max-w-md w-full bg-[#131b2e] border border-outline-variant rounded-xl p-6 shadow-2xl text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary mt-1.5 font-sans">
                      Piyasa akış sunucusuna bağlanırken beklenmeyen bir dahili sunucu hatası oluştu. Lütfen bağlantıyı yeniden deneyin.
                    </p>
                  </div>
                  <button
            ...
            <div className="flex-1 w-full my-4 relative">
              <svg className="w-full h-full" viewBox={`0 0 ${svgDimensions.width} ${svgDimensions.height}`} preserveAspectRatio="none">
                ...
                {chartType === 'candle' && chartCoordinates?.candles.map((c, idx) => (
                ...
                {/* Line graph drawing */}
                {chartType === 'line' && chartCoordinates && (
                  <path
        ...
        <div className="w-64 border-l border-outline-variant bg-bg-primary flex flex-col justify-between shrink-0">
          ...
          <div className="p-4 border-t border-outline-variant/30 bg-bg-card/30">
            ...
            <p className="text-[11px] text-text-secondary font-sans leading-relaxed">
              Analiz araçlarını kullanmak için sol araç çubuğunu kullanın. Ticker'a tıklayarak teknik mum grafiğini anında güncelleyebilirsiniz.
            </p>
          </div>
        </div>
      </div>
