...
import { X, Sparkles, TrendingUp, AlertTriangle, ArrowUpRight, ArrowDownRight, RefreshCw, BarChart2, Briefcase, HelpCircle } from 'lucide-react';
...
export default function StockDetailModal({ stock, onClose, onOpenTradeModal }: StockDetailModalProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage(error.message || 'AI Raporu oluşturulamadı. Lütfen API anahtarınızı kontrol edin.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center bg-bg-card/25 border border-outline-variant/40 p-4 rounded-xl">
                {/* Price tag */}
                <div>
                  <span className="text-[10px] font-label-caps text-text-muted block uppercase">Mevcut Enstrüman Değeri</span>
                ...
                <div className="md:col-span-2">
                  <div className="flex justify-between text-[10px] font-label-caps text-text-secondary mb-1">
                    <span>52H DÜŞÜK: ${stock.low52W.toFixed(2)}</span>
                    <span className="text-primary font-bold">52W SPREAD</span>
                    <span>52H YÜKSEK: ${stock.high52W.toFixed(2)}</span>
                  </div>
                  ...
                  <div className="text-right text-[9px] text-text-muted font-data-mono mt-1">
                    Mevcut seviye: {rangePercent.toFixed(0)}% (yıllık zirveye yakınlığı)
                  </div>
                </div>
              </div>
              ...
              <div className="bg-bg-card border border-outline-variant/30 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center border-b border-outline-variant/20 pb-2">
                  <span className="text-[10px] font-label-caps text-text-muted uppercase">60 Günlük Fiyat Trendi</span>
                  <span className="text-[10px] font-data-mono text-text-muted">
                    Min: ${minPrice.toFixed(2)} • Maks: ${maxPrice.toFixed(2)}
                  </span>
                </div>
              ...
              <div>
                <h4 className="text-[10px] font-label-caps text-text-muted tracking-wider uppercase border-b border-outline-variant/30 pb-1.5 mb-3">
                  Yatırım Değerleme Oranları (Financial Ratios)
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">P/E ORANI</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">{stock.pe.toFixed(2)}</span>
                  </div>
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">P/B ORANI</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">{stock.pb.toFixed(2)}</span>
                  </div>
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">DEBT / EQUITY</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">{stock.debtEquity.toFixed(2)}</span>
                  </div>
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">ROE VERİMLİLİK</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">%{stock.roe.toFixed(1)}</span>
                  </div>
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">BÜYÜME % (YOY)</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">%{stock.revenueGrowth.toFixed(1)}</span>
                  </div>
                  <div className="bg-bg-card border border-outline-variant/35 p-3 rounded-lg text-center">
                    <span className="text-[9px] font-label-caps text-text-muted block">TEMETTÜ VERİMİ</span>
                    <span className="font-data-mono text-sm font-bold text-text-primary mt-1 block">%{stock.divYield.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="bg-bg-card border border-outline-variant/30 rounded-xl p-6 space-y-4">
                <h4 className="text-[11px] font-label-caps text-text-muted tracking-wider uppercase border-b border-outline-variant/30 pb-2 flex items-center gap-1.5">
                  ...
                  <span>Teknik Analiz Göstergeleri</span>
                </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-bg-base/40 border border-outline-variant/20 rounded-lg p-5 flex flex-col items-center justify-center text-center">
                    <span className="text-[9px] font-label-caps text-text-muted mb-2">GENEL SİNYAL</span>
                    ...
                    <p className="text-[11px] text-text-secondary mt-3 leading-relaxed">
                      Hareketli ortalamalar ve osilatörlerin çoğu pozitif ivmeyi teyit ediyor.
                    </p>
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-3 font-sans text-xs">
                    <div className="flex justify-between py-2 border-b border-outline-variant/20 items-center">
                      <span className="text-text-secondary font-medium">RSI Göstergesi (14 Günlük):</span>
                      <span className="font-data-mono font-bold text-primary bg-primary/5 px-2 py-0.5 rounded">
                        {stock.technicals.rsi} - {stock.technicals.rsiStatus}
                      </span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-outline-variant/20 items-center">
                      ...
                      <span className="font-data-mono font-bold text-bull-green bg-bull-green/5 px-2 py-0.5 rounded">
                        {stock.technicals.macd} - {stock.technicals.macdStatus}
                      </span>
                    </div>
                    <div className="flex justify-between py-2 items-center">
                      <span className="text-text-secondary font-medium">SMA 50 Günlük Destek Seviyesi:</span>
                      <span className="font-data-mono font-bold text-text-primary">
                        ${stock.technicals.sma50.toFixed(2)} ({stock.technicals.sma50Status})
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-primary-container/5 border border-primary/10 rounded-lg text-xs text-text-secondary leading-relaxed">
                  <strong>Teknik Analiz Notu:</strong> RSI değerinin 30\'un altına inmesi aşırı satım (al sinyali), 70\'in üzerine çıkması ise aşırı alım (sat sinyali) durumunu gösterir. MACD ise trend yönünü teyit eden gecikmeli bir göstergedir.
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="bg-bg-card border border-outline-variant/30 rounded-xl p-6 space-y-5">
                <h4 className="text-[11px] font-label-caps text-text-muted tracking-wider uppercase border-b border-outline-variant/30 pb-2 flex items-center gap-1.5">
                  ...
                  <span>Konsensüs Analist Tavsiyesi</span>
                </h4>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                  <div className="space-y-4">
                    <div className="flex justify-between items-baseline">
                      <span className="text-xs text-text-secondary">Konsensüs 12 Aylık Hedef Fiyat:</span>
                      <span className="font-data-mono text-2xl font-bold text-primary">${stock.analystRating.targetPrice.toFixed(2)}</span>
                    </div>
                    <p className="text-xs text-text-secondary leading-relaxed">
                      Wall Street ve küresel aracı kurumlardan derlenen son 30 analist raporuna göre hedef fiyat medyanı mevcut fiyata göre <strong>%{(((stock.analystRating.targetPrice - stock.price) / stock.price) * 100).toFixed(1)}</strong> yukarıdadır.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 bg-bg-base/40 p-4 rounded-xl border border-outline-variant/20">
                    <div className="flex justify-between text-xs font-sans">
                      <span className="text-bull-green font-bold">AL (BUY / OVERWEIGHT):</span>
                      <span className="font-data-mono text-bull-green font-bold">{stock.analystRating.buyPercent}%</span>
                    </div>
                    <div className="flex justify-between text-xs font-sans">
                      <span className="text-warning-amber font-bold">TUT (HOLD):</span>
                      <span className="font-data-mono text-warning-amber font-bold">{stock.analystRating.holdPercent}%</span>
                    </div>
                    <div className="flex justify-between text-xs font-sans">
                      <span className="text-bear-red font-bold">SAT (SELL):</span>
                      <span className="font-data-mono text-bear-red font-bold">{stock.analystRating.sellPercent}%</span>
                    </div>
                    
                    <div className="w-full h-2.5 bg-bg-base rounded-full overflow-hidden flex mt-2">
                      <div className="bg-bull-green h-full" style={{ width: `${stock.analystRating.buyPercent}%` }} />
                      <div className="bg-warning-amber h-full" style={{ width: `${stock.analystRating.holdPercent}%` }} />
                      <div className="bg-bear-red h-full" style={{ width: `${stock.analystRating.sellPercent}%` }} />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between border-b border-outline-variant/30 pb-2 z-10 relative">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="font-headline text-sm font-bold text-text-primary uppercase tracking-wide">
                      Gemini AI Profesyonel Yatırım Raporu
                    </h4>
                  </div>
                ...
                {isLoading ? (
                  <div className="py-12 flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="font-sans text-xs text-text-secondary animate-pulse">
                      Gemini 3.5 Flash derin finansal verileri analiz ediyor, lütfen bekleyin...
                    </p>
                  </div>
                ) : errorMessage ? (
                  <div className="p-3 bg-bear-red/10 border border-bear-red/25 rounded-lg flex items-start gap-2.5">
                    ...
                    <div className="text-xs text-bear-red font-sans">
                      <strong>Bağlantı Hatası:</strong> {errorMessage}
                      <button
                        onClick={handleGenerateReport}
                  ...
                  <div className="bg-bg-base/40 border border-outline-variant/20 p-4 rounded-xl prose max-w-none">
                    ...
                    <div className="mt-4 pt-3 border-t border-outline-variant/20 flex justify-between items-center text-[9px] text-text-muted font-data-mono">
                      ...
                      >
                        <RefreshCw className="w-3 h-3" /> YENİDEN OLUŞTUR
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-6 text-xs text-text-secondary font-sans leading-relaxed">
                    Nvidia, Microsoft gibi teknoloji liderlerinin P/E, borç/özkaynak, ROE ve gelir büyümesi gibi temel rasyolarına göre formüle edilmiş <strong>Wall Street Hedge Fund</strong> seviyesinde bir Türkçe analizi anında oluşturmak için yukarıdaki "Analiz Oluştur" butonuna basın.
                  </div>
                )}
              </div>
            </div>
